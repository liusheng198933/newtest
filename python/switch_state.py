...
class rule:
    ...
    def print_rule(self):
        ...
        if type(self.rtmp) == int:
            ...
            printstr.append("rule dpid: %s, prt: %d, action: %d, rtmp_min: %d, rtmp_max: %d, ttmp: %d, table_id: %d\n" % (str(self.dpid), self.priority, self.action, self.rtmp[0], self.rtmp[1], self.ttmp, self.table_id))
    ...
    def if_match_bin(self, flow_bin):
        # match field only includes ipv4_src(optional) and ipv4_dst
        if intersection(self.match_bin, flow_bin) == flow_bin:
            return True
        else:
...
class table:
    ...
    def get_rule(self, flow):
        ...
        flow_bin = match_parse(flow)
        for i in range(len(self.tb)):
            #if self.tb[i].get_prt() > rprt and self.tb[i].if_match_bin(flow_bin):
            if self.tb[i].if_match_bin(flow_bin):
