...
def name2grpc(K, name):
    if name.startswith("cs"):
        num = int(name.split('_')[1])

    if name.startswith("as"):
        num = pow((K/2),2)
        num = num + (K/2) * int(name.split('_')[1]) + int(name.split('_')[2])
...
def grpc2name(K, num):
    ...
    if num < pow((K/2),2):
        ...
        if num < pow((K/2),2) + (K/2)*K:
            dnum = num - pow((K/2),2)
            pod = dnum / (K/2)
            aggr = dnum % (K/2)
            name = "as_"+str(pod)+"_"+str(aggr)
        else:
            dnum = num - pow((K/2),2) - (K/2)*K
            pod = dnum / (K/2)
            edge = dnum % (K/2)
            name = "es_"+str(pod)+"_"+str(edge)
    return name
