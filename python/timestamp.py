...
def setTMP(old_path, new_path, flow, new_state, rule_set, clk, in_port):
    ...
    for i in (set(old_path) - set(new_path)):
        if i not in rule_set.keys():
            ...
            rule_set[i]['add'] = []
            rule_set[i]['del'] = []
        rule_set[i]['add'].append(rule(i, match, clk, clk, in_port[i], table_id, SBPRT))
...
def sb_rule_construct(old_path, new_path, flow, clk, in_port):
    ...
    for i in (set(old_path) - set(new_path)):
        if i not in sb_set.keys():
            ...
            sb_set[i]['add'] = []
            sb_set[i]['del'] = []
        sb_set[i]['del'].append(rule(i, match, clk, clk, in_port[i], table_id, SBPRT))
    return sb_set
...
def rule_construct_normal(old_path, new_path, flow, state, prt, out_port):
    ...
    if old_path:
        ...
        for i in intersect_set:
            ...
            rule_set[i]['add'].append(rule(i, match, rtmp_max, 0, out_port[i], table_id, prt))
...
def rule_construct(old_path, new_path, flow, state, prt, out_port_old, out_port_new, clk):
    rule_set = {}
    table_id = 0
    match = {}
    match['ipv4_dst'] = flow['ipv4_dst']
    ...
    return rule_set
            ...
            #if i == old_path[0]:
            #    rule_set[i]['add'].append(rule(i, match, 0, clk, out_port[i], table_id, prt))
            #else:
        ...
        #for i in intersect_set:
            ...
            #rule_set[i]['del'] = []
            #rext = state.get_table(i, table_id).get_rule(flow)
            #if rext.get_prt() == prt and intersection(rext.get_match_bin(), flow_bin):
            #        rule_set[i]['del'].append(rule(i, rext.get_match(), rext.get_rtmp(), rext.get_ttmp(), rext.get_action(), table_id, prt))
                #df = difference(rext.get_match_bin(), match_parse(flow))
                #if df:
                #    for j in df:
...
def rule_construct_cu(old_path, new_path, flow, state, prt, out_port, clk):
    ...
    if not old_path:
        for i in range(len(new_path)):
            ...
            if i == len(new_path)-1:
                rule_set[new_path[i]]['add'].append(rule(new_path[i], match, [clk, clk], 0, out_port[new_path[i]], table_id, prt))
            else:
                if i == 0:
                    rule_set[new_path[i]]['add'].append(rule(new_path[i], match, 0, clk, out_port[new_path[i]], table_id, prt))
                else:
                    rule_set[new_path[i]]['add'].append(rule(new_path[i], match, [clk, clk], clk, out_port[new_path[i]], table_id, prt))
        return rule_set
    else:
        ...
        first_rule[new_path[0]]['add'] = [rule(new_path[0], match, 0, clk, out_port[new_path[0]], table_id, prt)]
        first_rule[new_path[0]]['del'] = [rule(new_path[0], match, 0, clk-1, out_port[new_path[0]], table_id, prt)]
        ...
        for i in range(len(new_path)):
            if i >= intersect_set[0]:
                ...
                if i == 0:
                    ...
                    if i == len(new_path)-1:
                        rule_set[new_path[i]]['add'].append(rule(new_path[i], match, clk, 0, out_port[new_path[i]], table_id, prt))
                    else:
                        rule_set[new_path[i]]['add'].append(rule(new_path[i], match, clk, clk, out_port[new_path[i]], table_id, prt))
...
def rule_construct_coco_final(old_path, new_path, flow, state, prt, out_port_old, out_port_new, clk):
    ...
    if not old_path:
        ...
        for i in range(len(new_path)):
            ...
            rule_set[new_path[i]]['del'] = []
            if i == len(new_path)-1:
                rule_set[new_path[i]]['add'].append(rule(new_path[i], match, clk, 0, out_port_new[new_path[i]], table_id, prt))
            else:
                if i == 0:
                    rule_set[new_path[i]]['add'].append(rule(new_path[i], match, 0, clk, out_port_new[new_path[i]], table_id, prt))
                else:
        ...
        for j in range(new_path.index(intersect_set[0]), len(new_path)):
            ...
            if i == new_path[0]:
                rule_set_first[i]['add'].append(rule(i, match, 0, clk+1, out_port_new[i], table_id, prt-1))
                rule_set_second[i]['del'].append(rule(i, match, 0, clk+1, out_port_new[i], table_id, prt-1))
                rule_set_second[i]['add'].append(rule(i, match, 0, clk+1, out_port_new[i], table_id, prt+1))
                rule_set_third[i]['del'].append(rule(i, match, 0, clk+1, out_port_new[i], table_id, prt+1))
                rule_set_third[i]['add'].append(rule(i, match, 0, clk, out_port_new[i], table_id, prt+1))
                rule_set_fourth[i]['del'].append(rule(i, match, 0, clk, out_port_new[i], table_id, prt+1))
                rule_set_fourth[i]['add'].append(rule(i, match, 0, clk, out_port_new[i], table_id, prt))
            else:
                if i == new_path[len(new_path)-1]:
                    rule_set_first[i]['add'].append(rule(i, match, clk+1, 0, out_port_new[i], table_id, prt-1))
                    rule_set_second[i]['del'].append(rule(i, match, clk+1, 0, out_port_new[i], table_id, prt-1))
                    rule_set_second[i]['add'].append(rule(i, match, clk+1, 0, out_port_new[i], table_id, prt+1))
                    rule_set_fourth[i]['del'].append(rule(i, match, clk+1, 0, out_port_new[i], table_id, prt+1))
                    rule_set_fourth[i]['add'].append(rule(i, match, clk, 0, out_port_new[i], table_id, prt))
                else:
        ...
        for j in range(old_path.index(intersect_set[0]), len(old_path)):
            ...
            if i not in rule_set_third.keys():
                ...
                rule_set_third[i]['del'] = []
            if i == old_path[0]:
                rule_set_third[i]['del'].append(rule(i, match, 0, clk, out_port_old[i], table_id, prt))
            else:
                if i == old_path[len(old_path)-1]:
                    rule_set_third[i]['del'].append(rule(i, match, clk, 0, out_port_old[i], table_id, prt))
                else:
